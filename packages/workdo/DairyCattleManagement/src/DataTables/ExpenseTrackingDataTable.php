...
class ExpenseTrackingDataTable extends DataTable
{
    ...
    {
        $rowColumn = ['category','expense_date','description'];
        $dataTable = (new EloquentDataTable($query))
            ->addIndexColumn();
        $dataTable = $dataTable->editColumn('category', function ($row) {
            return ucwords($row->category);
        });
        $dataTable = $dataTable->editColumn('expense_date', function ($row) {
            return company_date_formate($row->expense_date);
        });
        $dataTable = $dataTable->editColumn('description', function ($row) {
            $url = route('expense_tracking.show', $row->id);
        ...
        if (
            \Laratrust::hasPermission('expense tracking edit') ||
            \Laratrust::hasPermission('expense tracking delete')
        ) {
            $dataTable->addColumn('action', function (ExpenseTracking $expense_tracking) {
                return view('dairy-cattle-management::expense_tracking.action', compact('expense_tracking'));
            });
    ...
    public function html(): HtmlBuilder
    {
        $dataTable = $this->builder()
            ->setTableId('dairy_expense_trackings-table')
    ...
    {
        return 'Expense_trackings_' . date('YmdHis');
    }
}

...
class HealthDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        $dataTable =  (new EloquentDataTable($query))
            ->addIndexColumn()
            ->editColumn('animal_id', function (Health $model) {
                return $model->animal_name;
            })
            ->editColumn('price', function (Health $model) {
                return currency_format_with_sym($model->price);
            });

        if (\Laratrust::hasPermission('health show') || \Laratrust::hasPermission('health edit') || \Laratrust::hasPermission('health delete')) {
            $dataTable->addColumn('action', function (Health $health) {
                return view('dairy-cattle-management::health.action', compact('health'));
            });
            $rowColumn[] = 'action';
        }
     ...
     */
    public function query(Health $model): QueryBuilder
    {
        $healths = $model
    ...
    public function getColumns(): array
    {
        $column = [
            ...
            Column::make('veterinarian')->title(__('Veterinarien')),
            Column::make('duration')->title(__('Duration')),
            Column::make('date')->title(__('Date')),
            Column::make('checkup_date')->title(__('Next Checkup Date')),
            Column::make('diagnosis')->title(__('Diagnosis')),
            Column::make('treatment')->title(__('Treatment')),

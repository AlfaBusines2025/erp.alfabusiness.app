@permission('sales-distribution edit')
    <div class="action-btn me-2">
        <a class="mx-3 btn bg-info btn-sm align-items-center" data-bs-toggle="tooltip" data-bs-placement="top"
            title="{{ __('Edit') }}" data-ajax-popup="true" data-size="md" data-title="{{ __('Edit Sales Distribution') }}"
            data-url="{{ route('sales_distribution.edit', $sales_distribution->id) }}"><i class="text-white ti ti-pencil"></i></a>
    </div>
@endpermission

@permission('sales-distribution delete')
    <div class="action-btn">
        {{ Form::open(['route' => ['sales_distribution.destroy', $sales_distribution->id], 'class' => 'm-0']) }}
        @method('DELETE')
        <a href="#" class="mx-3 bg-danger btn btn-sm align-items-center bs-pass-para show_confirm" data-bs-toggle="tooltip"
            ...
            data-confirm-yes="delete-form-{{ $sales_distribution->id }}"><i class="text-white ti ti-trash"></i></a>
        {{ Form::close() }}
    </div>
@endpermission

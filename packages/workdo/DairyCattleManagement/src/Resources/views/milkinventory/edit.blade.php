...
@section('content')
    <div class="row">
        {{ Form::model($milkinventory, ['route' => ['milkinventory.update', $milkinventory->id], 'method' => 'PUT', 'class' => 'w-100', 'enctype' => 'multipart/form-data', 'class' => 'needs-validation', 'novalidate']) }}
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group" id="customer-box">
                                ...
                                {{ Form::select('daily_milksheet_id[]', $animals, !empty($milkinventory->daily_milksheet_id) ? explode(',', $milkinventory->daily_milksheet_id) : null, ['class' => 'form-control daily_milksheet_id multi-select choices', 'multiple' => 'multiple', 'id' => 'daily_milksheet_id', 'placeholder' => 'Select Daily Milk Sheet']) }}
                            </div>
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <div class="form-icon-user">
                                    {{ Form::select('product_id', $product, null, ['class' => 'form-control', 'placeholder' => 'Enter Product Name', 'required' => 'required']) }}
                                </div>
                            </div>
                        </div>
        ...
        <div class="modal-footer">
            <input type="button" value="{{ __('Cancel') }}"
                onclick="location.href = '{{ route('milkinventory.index') }}';" class="me-2 btn btn-light">
...
@push('scripts')
    <script>
        $(document).on('change', '.daily_milksheet_id', function() {
            ...
            dailymilksheet(animal_id);
        });
        $(document).ready(function() {
            var animal_id = $('#daily_milksheet_id').val();
            dailymilksheet(animal_id);
        });


        function dailymilksheet(animal_id) {
            var sum = [];

{{ Form::open(['url' => 'milkproduct', 'enctype' => 'multipart/form-data','class'=>'needs-validation','novalidate']) }}
                    ...
                    <select name="milk_inventory_id" class="form-control" required>
                        ...
                        @foreach ($animalsGrouped as $milkInventoryId => $animalNamesArray)
                            @foreach ($animalNamesArray as $animalNames)
                                <option value="{{ $milkInventoryId }}">{{ $animalNames }}</option>
                            @endforeach
                        @endforeach
                    </select>
                </div>
...
<script>
    $(document).ready(function() {
        // Listen for changes in the animal selection dropdown
